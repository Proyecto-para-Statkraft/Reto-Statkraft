...
const Twelfth = ({ data, Twelfth }) => {
    ...
    if (Twelfth !== "") {
        return (
          ...
          los casos de dolo o culpa inexcusable de{" "}
                    <span>{companyData.name}</span>, tal como lo establecen los artículos
          1328° y 1986° del Código Civil. Asimismo, la responsabilidad total de{" "}
                    <span>{companyData.name}</span> frente al <span>PROVEEDOR</span>
          ...
          mantener indemne y libre de todo perjuicio a{" "}
                    <span>{companyData.name}</span> y/o a sus afiliadas, subsidiarias,
          ...
          los bienes de <span>{companyData.name}</span>, del{" "}
                    <span>PROVEEDOR</span> o de terceros), responsabilidad, pérdida,
          ...
          COES o por cualquier otra autoridad administrativa contra{" "}
                    <span>{companyData.name}</span> por causas imputables al{" "}
                    <span>PROVEEDOR</span>.{" "}
                </p>
                <p>
          ...
          <span>PROVEEDOR </span> declara conocer y haber hecho conocer a su
                                        ...
                                        de Trabajo de
          <span>{companyData.name}</span>, el Reglamento Interno de Seguridad de{" "}
                    <span>{companyData.name}</span>, el Reglamento de Seguridad y Salud en
                    el Trabajo con Electricidad, R.M. N° 111-2013-MEM/DM, las normas y
          procedimientos de uso correcto de los servicios y equipos que{" "}
                    <span>{companyData.name}</span>
                    ponga a disposición del <span>PROVEEDOR</span>, así como todas las
          medidas para el cuidado de la seguridad, salud y medio ambiente que{" "}
                    <span>{companyData.name}</span> exige a sus contratistas. El{" "}
                    <span>PROVEEDOR</span> se compromete a observar y cumplir todas las
                    normas y reglamentos mencionados en este literal, así como todas
                    aquellas que
          <span>{companyData.name}</span> ponga en su conocimiento.
          Adicionalmente, el <span>PROVEEDOR</span>
                    ...
                    realizar dentro de las áreas operativas de
          <span>{companyData.name}</span>. <span>{companyData.name}</span> se
                                        reserva el derecho de impedir el ingreso de cualquier trabajador o
          ...
          operativas de <span>{companyData.name}</span>, el{" "}
                    <span>PROVEEDOR</span> se obliga a mantener libre de responsabilidad
          ...
          o invitados dentro o fuera de la propiedad o instalaciones de{" "}
                    <span>{companyData.name}</span>, o por cualquier daño originado en
          ...
          fiscalización que pueda verificarse por parte de{" "}
                    <span>{companyData.name}</span>, no existirá relación de dependencia
          ...
          infracción a las leyes y reglamentos por parte del{" "}
                    <span>PROVEEDOR</span>.
        </p>
                <p>
                    12.7 En el hipotético caso que alguna autoridad ordenara a{" "}
                    <span>{companyData.name}</span> el pago de suma alguna o la asunción
          ...
          reembolsando el monto correspondiente a{" "}
                    <span>{companyData.name}</span>. En este último supuesto, el{" "}
                    <span>PROVEEDOR</span> contará con un plazo de tres (3) días
          calendario contados a partir de la fecha en que{" "}
                    <span>{companyData.name}</span> le informase sobre el pago efectuado
          ...
          importes correspondientes y/o repetir dichos pagos contra el{" "}
                    <span>PROVEEDOR</span>.
        </p>
                <p>
                    12.8 El <span>PROVEEDOR</span> será responsable de reparar cualquier
          error en el funcionamiento de los Bienes en un plazo no mayor a{" "}
                    {numberHoursRepairLima} horas para Lima y a{" "}
                    {numberHoursRepairProvincia}
                    horas para provincias, si se determinase su responsabilidad respecto
          ...
          ios generados a <span>{companyData.name}</span> en caso de producirse
                                        un mal funcionamiento en los Bienes que se encuentren dentro del
                                        periodo de garantía.
        </p>
            </section>
        );
